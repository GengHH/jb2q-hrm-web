 ...
 * @LastEditTime: 2021-03-12 14:53:06
...
function signature(srcData, pinCode) {
    ...
    if (SecCtrl.KS_GetLastErrorCode() != 0 && SecCtrl.KS_GetLastErrorMsg() != "成功") {
        //alert(SecCtrl.KS_GetLastErrorMsg());
        alert("签名错误，" + SecCtrl.KS_GetLastErrorCode() + SecCtrl.KS_GetLastErrorMsg());
        return;
    }
    return {
        "signature": signature,
...
function checkPass(pinCode) {
    ...
    if (nRet == 0)
        ...
        if (nRet<=10)
            alert("密码错误, 还可重试" + nRet + "次！");
        else
            alert("登陆出错，请检查驱动和key是否正常！");
    return false;
}
