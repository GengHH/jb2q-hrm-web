/*
 * @Author: your name
 * @Date: 2021-03-12 11:47:09
 * @LastEditTime: 2021-03-12 13:34:04
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \jb2q-hrm-web\src\pages\adminLogin\module\SecCtrl.js
 */
////////////////////////////////////////////////////
//定义相关接口参数
// 非对称算法
...
// 印章解析项
var SEAL_ITEM_IMAGE = 0;					// 印章图片
var SEAL_ITEM_ISSUER_CERT = 1;				// 签发者证书
var SEAL_ITEM_SUBJECT_CERT = 2;				// 使用者证书
var SEAL_ITEM_NOT_BEFORE = 3;				// 生效时间
var SEAL_ITEM_NOT_AFTER = 4;				// 失效时间
var SEAL_ITEM_CREATE_TIME = 5;				// 创建时间
var SEAL_ITEM_ES_ID = 6;					// 印章ID
var SEAL_ITEM_SEAL_CN = 11;					// 印章使用者名称
////////////////////////////////////////////////////
...
function GetSecCtrl() {
    if (window.ActiveXObject || "ActiveXObject" in window) {
        document.write("<object id=\"SecCtrlCOM\" classid=\"clsid:17F8D3CF-857C-4D7C-9355-7A2398930B6A\" hidden=\"true\" width=\"0\" height=\"0\" style=\"display:none\"></object>");
...
function EnumProvider(provider) {
    ...
    elem1.value = "SKF&SKFAPI20046.dll";
    elem1.text = "海泰USB_SM2";
    provider.add(elem1);

    var elem2 = document.createElement("option");
    elem2.value = "SHCA-HaiTai Cryptographic Service Provider";
    elem2.text = "海泰USB_RSA";
    provider.add(elem2);


    var elem3 = document.createElement("option");
    elem3.value = "soft_file";
    elem3.text = "soft_file";
    provider.add(elem3);
...
function SecCtrlSS() {
    ...
    function HttpSend(json) {
        ...
        if (typeof window.toCheckCert !== "undefined" && window.toCheckCert) {
            window.toCheckCert = false;
            _xmlhttp.send(json);
            window.toCheckCert = true;
        }
        else
            _xmlhttp.send(json);
    };
    ...
    // 生成P10
    this.KS_MakeP10 = function (dn, alg) {
        var request = '{"F":7,"P":{"DN":"' + encodeURIComponent(dn) + '","Alg":' + encodeURIComponent(alg) + '}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 保存证书
    this.KS_SaveCert = function (signCert, encCert, encKey) {
        var request = '{"F":8,"P":{"SignCert":"' + encodeURIComponent(signCert) + '","EncCert":"' + encodeURIComponent(encCert) + '","EncKey":"' + encodeURIComponent(encKey) + '"}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 保存证书链
    this.KS_SaveCertChain = function (certChain) {
        var request = '{"F":9,"P":{"CertChain":"' + encodeURIComponent(certChain) + '"}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 修改密码
    this.KS_ModifyPin = function (oldPin, newPin) {
        var request = '{"F":31,"P":{"OldPin":"' + encodeURIComponent(oldPin) + '","NewPin":"' + encodeURIComponent(newPin) + '"}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 读取印章
    this.KS_GetSeal = function (num) {
        var request = '{"F":28,"P":{"Num":' + num + '}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 解析印章
    this.KS_GetSealItem = function (seal, type) {
        var request = '{"F":27,"P":{"Seal":"' + encodeURIComponent(seal) + '","Type":' + type + '}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
    ...
    // 解析印章
    this.KS_VerifySeal = function (seal) {
        var request = '{"F":28,"P":{"Seal":"' + encodeURIComponent(seal) + '"}}';
        try {
            HttpSend(request);
        } catch (e) {
            return -1;
