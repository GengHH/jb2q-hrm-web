 ...
 * @LastEditTime: 2021-03-17 09:58:40
 * @LastEditors: GengHH
...
function GetSecCtrl() {
  if (window.ActiveXObject || 'ActiveXObject' in window) {
    document.write(
      '<object id="SecCtrlCOM" classid="clsid:17F8D3CF-857C-4D7C-9355-7A2398930B6A" hidden="true" width="0" height="0" style="display:none"></object>'
    );
...
function SecCtrlSS() {
  ...
  this.KS_SaveCert = function(signCert, encCert, encKey) {
    var request =
      '{"F":8,"P":{"SignCert":"' +
      encodeURIComponent(signCert) +
      '","EncCert":"' +
      encodeURIComponent(encCert) +
      '","EncKey":"' +
      encodeURIComponent(encKey) +
      '"}}';
    try {
      HttpSend(request);
    } catch (e) {
  ...
  this.KS_SaveCertChain = function(certChain) {
    var request =
      '{"F":9,"P":{"CertChain":"' + encodeURIComponent(certChain) + '"}}';
    try {
      HttpSend(request);
    } catch (e) {
      return -1;
  ...
  this.KS_VerifyP7SignData = function(p7SignData) {
    var request =
      '{"F":18,"P":{"P7SignData":"' + encodeURIComponent(p7SignData) + '"}}';
    try {
      HttpSend(request);
    } catch (e) {
      return -1;
  ...
  this.KS_OpenEnvelope = function(envelope) {
    var request =
      '{"F":24,"P":{"Envelope":"' + encodeURIComponent(envelope) + '"}}';
    try {
      HttpSend(request);
    } catch (e) {
      return -1;
  ...
  this.KS_ModifyPin = function(oldPin, newPin) {
    var request =
      '{"F":31,"P":{"OldPin":"' +
      encodeURIComponent(oldPin) +
      '","NewPin":"' +
      encodeURIComponent(newPin) +
      '"}}';
    try {
      HttpSend(request);
    } catch (e) {
  ...
  this.KS_SymmEncryptFile = function(infile, outfile, key, alg) {
    var request =
      '{"F":29,"P":{"InFile":"' +
      encodeURIComponent(infile) +
      '","OutFile":"' +
      encodeURIComponent(outfile) +
      '","Key":"' +
      encodeURIComponent(key) +
      '","Alg":' +
      alg +
