  ...
  <div id="indexBody">
    ...
    <ttable :tableClass="'tableClass'" :columns="columns" :list="list0">
      ...
      <el-table-column slot="salary" label="薪酬" align="center">
        <template slot-scope="scope">
          {{
            (scope.row.salary = scope.row.salaryMin + '-' + scope.row.salaryMax)
          }}
        </template>
      </el-table-column>
      <el-table-column slot="workNature" label="工作性质" align="center">
        <template slot-scope="scope">
          <template v-for="(v, k) in dicOptions.nature">
            <el-tag :key="k" v-if="v.value == scope.row.workNature">
              {{ v.label }}
            </el-tag>
          </template>
        </template>
...
import gapdetails from './module/gapDetails';

import { lack_query, lack_result } from './api/index';
